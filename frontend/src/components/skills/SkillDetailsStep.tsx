...
const DELIVERY_ICONS: Record<(typeof DELIVERY_MODES)[number]["id"], LucideIcon> = {
  digital: Send,
  videocall: Video,
  chat: MessageCircle,
  presencial: MapPin,
  ...
  return (
    <section className="space-y-8">
      <div className={wizardSectionClass}>
        ...
        <div className="mt-4 flex gap-3 rounded-xl border border-border bg-white/80 px-4 py-3 sm:items-start">
      ...
      <div className={wizardSectionClass}>
        <div className="mb-3 flex items-center gap-2">
          ...
          <div>
            <p className="text-base font-bold tracking-tight text-foreground">Foto de portada</p>
      ...
      <div className={wizardSectionClass}>
        ...
        <div className="flex flex-wrap gap-2">
          {DELIVERY_MODES.map((mode) => {
            const selected = data.deliveryMode === mode.id;
            ...
            return (
              <Button
                ...
                type="button"
                variant="outline"
                className={`h-auto min-h-11 rounded-xl border-2 px-3.5 py-2 text-left text-sm font-semibold transition-[border-color,box-shadow,background-color] ${
                  selected
                    ? "border-primary bg-primary/10 text-primary shadow-sm ring-1 ring-primary/15"
                    : "border-border bg-surface-elevated text-foreground hover:border-primary/35 hover:bg-primary/[0.04]"
                }`}
                onPress={() => onFieldChange("deliveryMode", mode.id)}
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span className="leading-snug">{mode.label}</span>
                </span>
              </Button>
            );
          })}
        ...
        {errors.deliveryMode ? (
          <p className="mt-2 text-xs font-medium text-accent-red">{errors.deliveryMode}</p>
        ) : null}
      </div>
      ...
      <div className={`${wizardSectionClass} border-primary/15 bg-gradient-to-br from-primary/[0.06] to-transparent`}>
        ...
        {data.promoEnabled ? (
          <div className="space-y-5 border-t border-primary/10 pt-5">
            <p className="rounded-lg border border-primary/15 bg-white/80 px-3 py-2 text-xs leading-relaxed text-muted">
              Primero el <strong className="text-foreground">precio normal</strong> (el más alto), luego el{" "}
              <strong className="text-foreground">precio en oferta</strong> (más bajo). Si ya tenías un precio
              guardado, puedes usarlo como oferta y subir el normal arriba de ese valor.
            </p>
            <div className="grid gap-5 sm:grid-cols-2">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted">Lo que sueles cobrar o tu tarifa sin descuento (debe ser mayor que la oferta).</p>
                <Input
                  id="skill-list-price"
                  type="number"
                  min={MIN_PRICE + 1}
            ...
            {listOk && data.promoEndsAtLocal.trim() ? (
              <p className="flex items-start gap-2 rounded-xl border border-success/25 bg-success/10 px-3 py-2.5 text-xs leading-relaxed text-foreground">
                <span className="mt-0.5 inline-flex size-5 shrink-0 items-center justify-center rounded-full bg-success/20 text-success" aria-hidden>
                  ✓
                </span>
                <span>
